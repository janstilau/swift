...
///
/// 从这里可以看到, AsyncStream 使用起来, 有着很大的灵活性. 如果这是一个实例属性, 那么就是将 self 传递到 Block 里面, 来生成一个新的 AsyncStream, 而 Self 是可以传递出去在不同的线程里面进行 yield 调用的.
...
public struct AsyncStream<Element> {
    ...
    // 各种操作, 都是包装的 Storage 的实现.
    // 在 Swift 中, 各种拿到的类型, 都是抽象数据类型的体现而已.
    public struct Continuation: Sendable {
        // 在类型内部, 起始位置, 将需要用到的各种数据类型进行定义.
        // 这是一种非常常见的书写方案. 并且, 没有专门开启一个 Extension 作为定义的场所.
        ...
        // Continuation 就是封装了对于 _Storage 的相关操作而已.
        // 使用特殊的类型, 给与外界用户更加友好的接口, 也能避免 _Storage 的传出, 使得 _Storage 的内部状态收到破坏.
    ...
    // 没明白, 这个类的意义在哪里, 直接使用 _Storage 有什么问题.
    // 不过, 在官方的源码里面, 其实也是喜欢使用 _ 开头的类型命名. 变量命名变少了.
...
// 真正的, AsyncStream 实现部分.
// 对于 Sequence 来说, 就是进行 Iter 的构建而已.
// 而 Iter 则是真正的进行数据消耗的部分.
@available(SwiftStdlib 5.1, *)
extension AsyncStream: AsyncSequence {
    // Iter 仅仅是一个包装器而已, 真正的实现, 还是在 _Context 的内部.
