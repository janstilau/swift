...
/*
 之前的各种 Sequence 使用 makeIterator 返回的类型, 都是各个 Sequence 下专门定义的 Iterator 具体类型.
 类型消除器, 作用就是消除具体类型, 然后返回一个通用的数据类型. 这个通用的数据类型, 绑定 Ele 的数据类型. 外界直接使用 AnyIterator 就可以完成迭代这件事.
 */
...
/// 之前没有太了解, 这个东西的实际作用是什么. 类型消除器的最大作用, 就是消除迭代器的具体类型.
/*
 类型消除器的实现, 基本上都是一个套路.
 使用一个具体的类型,
 */
public struct AnyIterator<Element> {
    ...
    // AnyIterator 最主要的作用, 就是可迭代, 仅仅暴露要迭代的数据类型, 隐藏所有底层正在迭代的数据类型.
    ...
    // <I: IteratorProtocol> 是做限制用的, 不能当做一个实际的类型来进行使用.
    // 在真正使用的时候, 类型是确定了的
...
// 不是太明白, 为什么需要这样的一个 Base 类存在. 不过, 从语法的含义来说, 这个类绑定了 IteratorProtocol 中的 Element 类型.
// 真正的实现, 是 Box 实现.
internal class _AnyIteratorBoxBase<Element>: IteratorProtocol {
    ...
    // 官方的实现里面, 都是使用这种方式进行的虚函数的定义.
    @inlinable // FIXME(sil-serialize-all)
    internal func next() -> Element? { _abstract() }
}

// 这是真正的进行, BaseIterator 存储的地方.
// 所有的, 对于 IteratorProtocol 实现, 都是转移给了 BaseIterator.
@usableFromInline
...
// 真正的, 对于 AnySequence 中, 对于 Base Sequence 进行存储的地方.
internal final class _SequenceBox<S: Sequence>: _AnySequenceBox<S.Element> {
    // 使用 typealias, 更加清晰的方式, 来完成 Element 的绑定工作.
    ...
    // _SequenceBox 是一个内部类, 不会在外部使用, 所以它没有必要实现 Sequence 协议. 这里, 是定义了各种 Sequence 的方法, 在内部实现了即可.
    @inlinable
    ...
    // 最为核心的地方, Box 的作用, 就是存储一些外界传入的. <S: Sequence> 已确定类型的对象而已.
