...
public struct Mirror {
    ...
    /*
        这是, 暴露给外界的, 定义 Mirror 的构造方法.
        如果, 传递过来的对象, 定义了 customMirror, 那么就使用 customMirror 当做 Mirror.
        如果, 否则就使用 Mirror(internalReflecting:) 构造函数.
        在 Mirror(internalReflecting:) 里面, 包含了元类型数据解析的逻辑. 最终, 将解析完的数据, 赋值给 Mirror 的各个成员变量
     */
    ...
    /*
        实际上, Mirror 在这里, 仅仅是存值的工作.
        取出 Mirror 进行操作, 也是取到相应的数据结构, 直接使用相应的值.
        不过, Mirror 在初始化过程中, 进行了大量的解析元信息的工作 .
     */
    ...
    /*
     获取, SuperClassMirror, 是调用了 _makeSuperclassMirror 这个闭包, 这个闭包, 在 ReflectionMirror 文件里面有赋值.
     */
