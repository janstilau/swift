...
/*
 这个类型, 很好的表现除了, 泛型: 确定类型的功能
 泛型的使用:
 1. 编写通用的逻辑, 这些逻辑是建立协议的基础上, 这一点, C++ 也是同样的使用场景. 只不过是编译器进行的约束.
 2. 确定类型. 比如 Map 底层是 RBTree, 但是 RBTree 不直接暴露, Map 里面确定的类型, 会传递到 RBTree 中.
 这种传递的手法, 产生了一些中间类型, 这些中间类型就是为了将传递的类型, 包装到复杂的类型中.
 比如 YDPlugin 里面的各种 Template 类型.
 还有就是直接使用类型的方法, 比如这里, 类型其实就像是一个参数被传递进来了, 但是这个传递, 使用的时候是感觉不到的, 比如函数调用的时候, 参数的类型是直接编译器推导出来的. 函数调用实参类型推导这个技术, 产生了大量的函数, 例如 bind1st, 里面产生一个泛型类型的实例, 而这个实例的类型参数, 是编译器推到出来的.
 还有就是这里, MemoryLayout<Int>, 调用的时候手动填入明确的类型, 在 C++ 里面, 经常是直接使用这个类型的静态函数. 这种写法, 在使用的时候也是感觉不到类型参数的传递, 因为这不是函数调用的传递方式.
 这就是编译时多态的概念.
 */


// 实际上, 还是使用了 Builtin 里面的函数, 这个函数使用传递过去的类型信息, 返回类型信息对应的 size, stride, align 的值.
@frozen // namespace
