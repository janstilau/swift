...
public protocol Decoder {
    ...
    /// keyed by the given key type.
    // 这里, Key 必须是 CodeingKey 类型的. 因为, KeyedDecodingContainer 里面有了限制.
    func container<Key>(
...
public protocol KeyedEncodingContainerProtocol {
    ...
    /// Encodes the given value for the given key.
    // 标准库, 提供了所有的基本数据类型的 Encode 方法, 复杂的运算, 用户自己掌握.
...
// KeyedEncodingContainer 里面, 有个 _box: _KeyedEncodingContainerBase 对象,
// 所有的实现, 都移交给了 _box 对象
...
// KeyedEncodingContainer 中的实际实现类.
// _KeyedEncodingContainerBase 没有实现任何的功能, 所有的实现, 在 _KeyedEncodingContainerBox 中.
// private struct JSONKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol, _SpecialTreatmentEncoder
// 实现, 又移交给了 concrete: Concrete.
// 莫名其妙的抽象.
