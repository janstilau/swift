...
/*
 这是, 没有引入异步函数之前, 官方推荐的使用闭包这种方式, 作为异步操作的回调传值的做法.
...
public enum Result<Success, Failure: Error> {
    ...
    public func map<NewSuccess>(
        ...
        // 实现, 其实就是根据当前的 case 值, 进行变化.
        // 其实是类似于 Combine 里面, 传值的逻辑.
        switch self {
    ...
    // 下面的两个方法, 分别是将 Result 重新放置到 catch 体系里面,
    // 以及从 Catch 体系里面, 进行 Result 建立的过程.
