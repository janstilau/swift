...
///         var endIndex: Int   { return 2 } // EndIndex 返回的, 不是 1, 而是 1 的 next.
///
///         subscript(index: Int) -> Element { // 对于 Collection 来说, 根据 index 取值, 必须取到值. Index 的值, 应该是程序机算出来的. 这里和 Array 的思路是一样的.
///             switch index {
...
///         }
///
///         // 最最重要的方法, Collection 应该提供, Index 的后移操作.
...
public struct IndexingIterator<Elements: Collection> {
    // 对于 IndexingIterator 来说, 就是存值.
    // 存的直接就是 Collection 类型的抽象. 所以, 迭代器保留着对于所在容器的索引, 这一个惯例实现是一直存在的.
...
extension IndexingIterator: IteratorProtocol, Sequence {
    ...
    // Collection, 其实是保留了如何通过 Index 读取值的逻辑的.
    // Collection, 其实是保留了, 如何计算下一个 Index 值的逻辑的.
    // 整个 IndexingIterator 的实现逻辑, 是完全建立在 Collection 抽象的基础上的.
...
// 通过下标来访问数据, 这是 Collection 最最重要的操作了.
// 这里, 对于 EndIndex 的定义, 和之前的是完全一致的.
...
// Protocol 最有意义的部分, 就是可以进行各种各样的扩展.
...
// Index 是和 Collection 息息相关的, 本身就是 Collection 的内部存储结构的表现.
...
// Mutating 操作, 会让 Index 的值变得无效. 因为可能会引起内部的存储结构变化.
...
// Collection Slice 的概念, 是需要 Index Compareable 来进行的实现.
...
// Slice 这个概念是非常有用的, 他避免了复制操作.
// 在 OC 里面, 是没有这个概念的, 所以在 OC 里面, 每次传值都是 Copy.
...
// Slice 是和原始 Collection 共享 Index 的, 其实很简单, 就是记录了一下开始结尾两个位置而已.
