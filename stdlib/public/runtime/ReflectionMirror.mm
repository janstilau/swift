...
struct StructImpl : ReflectionMirrorImpl {
    ...
    // 这里, struct 的大部分的数据, 是放到了自己的 Description 类型里面
    char displayStyle() {
        return 's';
    }
...
template<typename F>
auto call(OpaqueValue *passedValue,
          const Metadata *InferredType,
          const Metadata *passedType,
          const F &f) -> decltype(f(nullptr))
{
    const Metadata *type;
    OpaqueValue *value;
    std::tie(type, value) = unwrapExistential(InferredType, passedValue);
...
// 这里, value 变成了指针了.
// _getNormalizedType 里面可能会传过去 struct 的, 为什么这里就是指针了.
